...
$result = mysqli_query($conn, $sql);
    ...
    echo("<h3> ZAD 1 </h3>");
$sql = "SELECT *,avg(zarobki) as srednia FROM pracownicy where (avg(zarobki)<35) and imie not like '%a' group by dzial";
    echo ("<li>".$sql."</li><br><br>");
$result = mysqli_query($conn, $sql);
    echo ('<table border = "1" class = "moja_tabelka">');
    echo ("<tr><th>imie</th><th>srednia</th><th>data_urodzenia</th><th>dzial</th></tr>");
        while ($row = mysqli_fetch_assoc($result)) {
                echo ('<tr>');
                echo ('<td>'.$row["imie"].'</td><td>'.$row["srednia"].'</td><td>'.$row["data_urodzenia"].'</td><td>'.$row["dzial"].'</td>');
                echo ('</tr>');
        }echo ('</table>');
    
        echo("<h3> ZAD 2 </h3>");
$sql = "SELECT imie, zarobki FROM pracownicy where id_org=dzial group by dzial having avg(zarobki)<40";
    echo ("<li>".$sql."</li><br><br>");
$result = mysqli_query($conn, $sql);
    echo ('<table border = "1" class = "moja_tabelka">');
